...
export default function Home() {
    ...
    function loadInitData() {
        axios.get('http://192.168.25.163:5000/devices_v3')
            .then(response => {
        ...
        devices.forEach((device) => { //Laço para percorrer todos os dispositivos existentes
            if (device.key === searchKey) { //Se a a chave recebida for igual a lida atualmente, inverte o estado do dispositivo atual
                ...
                if (device.expanded === false) {
                    //loadDataForGraphic();
                    loadGraphicData(searchKey);
                    alert("Dados para o gráfico" + JSON.stringify(dados));
                    getFrequency(1);
    ...
    function loadDataForGraphic() {
        axios.get("http://192.168.25.163:5000/dados_graph")
            .then(response => {
    ...
    async function loadGraphicData(searchKey) {//Teste
        ...
        try {
            response = await axios.post('http://192.168.25.163:5000/temperatures_v3', params);
            //alert("JAVASCRIPT PALHAÇO: " + JSON.stringify(response.data.ocorrencias));
            //alert("Resposta temperatures: " + JSON.stringify(response.data));
            let horaRegistrada = [], dataRegistro = [], temperatura = [];
            response.data.ocorrencias.map((ocorr) => {
                horaRegistrada.push(ocorr.horaRegistrada + " " + ocorr.dataRegistro);
                temperatura.push(ocorr.temperatura);
            })
            //alert(JSON.stringify(horaRegistrada));
            ...
            setDados(graphicData);
            //setDados(response.data.ocorrencias);
            ...
            <LinearGradient
                ...
                <ScrollView style={{ height: '82.5%', marginTop: 15 }}>
                    ...
                    {devices.length < 1 ?
                        (
                            <View style={{alignItems: "center"}}>
                                <Text style={{color: '#FFF', fontSize: 25, textAlign: "center", paddingTop: 25}}>ATENÇÃO!</Text>
                                <Text style={{color: '#FFF', fontSize: 18, textAlign: "center", padding: 25}}>Nenhum dispositivo encontrado. Se você tem certeza de que existem dispositivos cadastrados, por favor, verifique sua conexão com a internet ou o estado do servidor.</Text>
                                <TouchableOpacity style={{ height: 50, width: 0.65 * width, flexDirection: "row", borderRadius: 25, backgroundColor: '#FFF', shadowColor: '#000', elevation: 10 }} onPress={() => { loadInitData(); }}>
                                    {/* Os botões terão um Gradient também */}
                                    <LinearGradient
                                        start={{ x: 0, y: 0 }}
                                        end={{ x: 1, y: 0 }}
                                        colors={['#F28705', '#F2E205']}
                                        style={{ borderRadius: 255, width: '100%', flexDirection: "row", alignItems: "center", justifyContent: "center" }}
                                    >
                                        <Text style={{width: '65%', color: '#FFF', textAlign: "right", fontSize: 18, marginRight: 5}}>Recarregar</Text>
                                        <Icon style={{width: '35%', textAlign: "left"}} name="redo-alt" size={18} color={'#FFF'}></Icon>
                                    </LinearGradient>
                                </TouchableOpacity>
                            </View>
                        )
