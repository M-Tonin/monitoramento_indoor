...
uint8_t tempHIGH;
uint8_t tempLOW;
// Valor de lux e bytes mais e menos significativos respectivamente
uint16_t lux;
uint8_t luxHIGH;
uint8_t luxLOW;
// Contantes auxiliáres
unsigned long int intervalo = 10;
int cont = 0;
...
{        
 ...
 //Serial.print(", Valor de Resistência Sensor luz:"); Serial.print(resistance); Serial.println("Ohm");
  ...
  // Dividindo os 2 Bytes recolhidos de lux em 1 Byte 
  // para cada variável para o envio do dado

    luxHIGH = (lux & 0xFF00) >> 8;
    luxLOW = (lux & 0x00FF);

  // Dividindo os 2 Bytes recolhidos de temperatura em 1 Byte 
  // para cada variável para o envio do dado
    tempHIGH = (temp & 0xFF00) >> 8;
    tempLOW = (temp & 0x00FF);
 ...
 Serial.println("---------------------------------------");
  ...
  do{
    ...
    cont = cont +1;
  }while(digitalRead(BOTAO) == HIGH && cont < 1000 );
  Serial.print("Terminou Loop\n");
  //Impressão dos valores do botão e do tempo decorrido em segundos
  Serial.println(cont*intervalo/1000);
